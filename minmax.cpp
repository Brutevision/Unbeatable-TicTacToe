...
using namespace std;
int index1;
char board[9] = {'*','*','*','*','*','*','*','*','*'}; // Single array representing the board. '#' means empty box :)
...
{
   int move,choice;
   cout<<"---------------------------------------------------TRY DEFEATING ME---------------------------------------------------" << endl;
   cout<<"USER--->(O)      CPU------>(X)"<<endl;
   cout<<"SELECT : 1-> Player first 2-> CPU first:";
   cin>>choice;
   if(choice == 1)
   {
       draw_board();
      up:cout<<endl<<"Enter the move:";
         ...
         {
             cout<<endl<<"Move not allowed, try again";
             goto up;
         }
   }
